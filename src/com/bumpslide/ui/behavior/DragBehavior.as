...
package com.bumpslide.ui.behavior {
	...
	public class DragBehavior {
		...
		protected function handleMouseDown(event:MouseEvent):void {
			...
			dragTarget.dispatchEvent(new DragEvent(DragEvent.EVENT_DRAG_START, spriteStart, null));			
		}

		protected function whileDragging(event:MouseEvent):void {		
			//trace( this + ' whileDragging() ' );				
			var delta:Point = new Point(event.stageX - mouseStart.x, event.stageY - mouseStart.y);
			...
			delta = m.deltaTransformPoint( delta ); // ignore tx and ty 
			...
			dragTarget.dispatchEvent(new DragEvent(DragEvent.EVENT_DRAG_MOVE, spriteStart, delta));
		...
		public function stopDragging(event:Event = null):void {			
			...
			dragTarget.dispatchEvent(new DragEvent(DragEvent.EVENT_DRAG_STOP, spriteStart, null));
			spriteStart = null;
			if(dragTarget is DisplayObjectContainer) { 
